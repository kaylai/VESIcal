...
class water(model_classes.Model):
    ...
    def __init__(self):
        ...
        self.set_activity_model(activity_models.activity_idealsolution())
        self.set_calibration_ranges([calibration_checks.CalibrationRange('pressure',1000,calibration_checks.crf_LessThan,'bar','Dixon (1997, Pi-SiO2 simpl.) Water',
                                     ...
                                     calibration_checks.CalibrationRange('pressure',2000,calibration_checks.crf_LessThan,'bar','Dixon (1997, Pi-SiO2 simpl.) Water',
                                                      ...
                                                      pass_msg=calibration_checks.crmsg_LessThan_pass,
                                                      description_msg=calibration_checks.crmsg_LessThan_description),
                                     calibration_checks.CalibrationRange('SiO2',49,calibration_checks.crf_LessThan,'wt%','Dixon (1997, Pi-SiO2 simpl.) Water',
                                                      fail_msg=crmsg_49_fail,
                                                      pass_msg=calibration_checks.crmsg_LessThan_pass,
                                                      description_msg=calibration_checks.crmsg_LessThan_description),
                                     calibration_checks.CalibrationRange('SiO2',40,calibration_checks.crf_GreaterThan,'wt%','Dixon (1997, Pi-SiO2 simpl.) Water',
                                                      fail_msg=crmsg_40_fail, pass_msg=calibration_checks.crmsg_GreaterThan_pass,
                                                      description_msg=calibration_checks.crmsg_GreaterThan_description),
                                     calibration_checks.CalibrationRange('temperature',[1000,1400],calibration_checks.crf_Between,'oC','Dixon (1997, Pi-SiO2 simpl.) Water',
                                                       fail_msg=crmsg_BC_T,
    ...
    def root_saturation_pressure(self,pressure,sample,kwargs):
        ...
        float
            ...
            passed in the sample variable.
        """
        return self.calculate_dissolved_volatiles(pressure=pressure,sample=sample,**kwargs) - sample.get_composition('H2O')

crmsg_40_fail="{param_name} ({param_val:.1f} {units})<40 wt%, which is the lower calibration limit of the Dixon (1997, Pi-SiO2 simpl.) Model. VESIcal has performed calculations assuming SiO2=40wt%. "
crmsg_1000bar_fail="{param_name} exceeds 1000 bar, which Iacono-Marziano et al. (2012) suggest as an upper calibration limit of the Dixon (1997, Pi-SiO2 simpl.) Model, "
crmsg_2000bar_fail="as well as the upper calibration limit of 2000 bar suggested by Lesne et al. (2011), "
crmsg_5000bar_fail="and the upper calibration limit of 5000 bar suggested by Newman and Lowenstern, (2002). "
crmsg_49_fail="{param_name} ({param_val:.1f} {units})>49 wt%, which is the calibration limit of the Dixon (1997, Pi-SiO2 simpl.) Model. VESIcal has performed calculations assuming SiO2=49wt% for this sample. "
crmsg_BC_T="{param_name} ({param_val:.1f} {units}) lies more than 200°C away from the temperature the Dixon (1997, Pi-SiO2 simpl.) model was calibrated for (the range suggested by Newman and Lowenstern, 2002; VolatileCalc). " # Warning for Dixon temperature
