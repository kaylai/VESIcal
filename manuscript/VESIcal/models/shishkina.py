...
class carbon(model_classes.Model):
    ...
    def __init__(self):
        ...
        self.set_calibration_ranges([calibration_checks.CalibrationRange('pressure',[500.0,5000.0],calibration_checks.crf_Between,'bar','Shishkina et al. carbon',
                                     ...
                                     calibration_checks.CalibrationRange('temperature',[1200.0,1300.0],calibration_checks.crf_Between,'oC','Shishkina et al. carbon',
                                                       fail_msg=calibration_checks.crmsg_Between_fail,
                                                      pass_msg=calibration_checks.crmsg_Between_pass,
                                                      description_msg=calibration_checks.crmsg_Between_description),
                                     calibration_checks.CalibrationRange('SiO2',[40,57],calibration_checks.crf_Between,'wt%','Shishkina et al. carbon',
...
class water(model_classes.Model):
    ...
    def __init__(self):
        ...
        self.set_activity_model(activity_models.activity_idealsolution())
        self.set_solubility_dependence(False)
        self.set_calibration_ranges([calibration_checks.CalibrationRange('pressure',[500.0,5000.0],calibration_checks.crf_Between,'bar','Shishkina et al. water',
                                                      fail_msg=calibration_checks.crmsg_Between_fail,
                                                      pass_msg=calibration_checks.crmsg_Between_pass,
                                                      description_msg=calibration_checks.crmsg_Between_description),
                                    calibration_checks.CalibrationRange('temperature',[1050,1400],calibration_checks.crf_Between,'oC','Shishkina et al. water',
                                                       fail_msg=crmsg_BC_fail_T1,
                                                      pass_msg=calibration_checks.crmsg_Between_pass,
                                                       description_msg=calibration_checks.crmsg_Between_description),
                                    calibration_checks.CalibrationRange('SiO2',65,calibration_checks.crf_LessThan,'wt%','Shishkina et al. water',
                                    ...
                                    calibration_checks.CalibrationRange('SiO2',40,calibration_checks.crf_GreaterThan,'wt%','Shishkina et al. water',
    ...
    def calculate_saturation_pressure(self,sample,**kwargs):
        ...
        if sample.get_composition('H2O') < self.calculate_dissolved_volatiles(sample=sample,pressure=0,**kwargs):
            return np.nan
...
crmsg_BC_fail_T1 = "{param_name} ({param_val:.1f} {units}) is outside the calibration range of {model_name} \
({calib_val0:.1f}-{calib_val1:.1f} {units}). Note, the authors recomend that this model is optimally calibrated between 1150-1250C. "
crmsg_BC_fail_C1="{param_name} ({param_val:.1f} {units}) is outside the calibration range of {model_name} (calculated\
 from the max and minimum concentrations in the calibration dataset +-5%; {calib_val0:.1f}-{calib_val1:.1f} {units})."
crmsg_GreaterThan_fail_WaterSi = "{param_name}{units} exceeds the lower calibration limit of {model_name} ({calib_val:.1f} \
{units}, based on the minimum concentration minus 5% in the calibration dataset). " # This check warns users if SiO2<40 in the water model
crmsg_LessThan_fail_WaterSi = "{param_name} ({param_val:.1f} {units}) exceeds the upper limit of {calib_val:.1f} {units} \
suggested by Shishkina et al. for their H2O model. " # This check warns users if SiO2>65 wt% , the upper limit suggested by Shishkina et al. (2014)



mixed = model_classes.MixedFluid({'H2O':water(),'CO2':carbon()})

mixed.models[0].calibration_ranges.append(calibration_checks.CalibrationRange('X_fluid',0.0,calibration_checks.crf_MixedFluidWarning,'','Shishkina et al. (2014)',
