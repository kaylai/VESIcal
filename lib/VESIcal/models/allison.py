...
class carbon(model_classes.Model):
    ...
    def __init__(self,model_loc='sunset',model_fit='thermodynamic'):
        ...
        self.set_calibration_ranges([calibration_checks.CalibrationRange('temperature',1200,calibration_checks.crf_EqualTo,'oC','Allison et al. (2019) carbon',
                                                       fail_msg=crmsg_Temp,
                                                      pass_msg=calibration_checks.crmsg_EqualTo_pass,
                                                      description_msg=calibration_checks.crmsg_EqualTo_description),
                                    calibration_checks.CalibrationRange('temperature',[1000,1400],calibration_checks.crf_Between,'oC','Allison et al. (2019) carbon',
                                                                                   fail_msg=crmsg_Between_Temp,
                                                                                  pass_msg=calibration_checks.crmsg_EqualTo_pass,
                                                                                  description_msg=calibration_checks.crmsg_EqualTo_description),
                                    calibration_checks.CalibrationRange('H2O',0.5,calibration_checks.crf_LessThan,'wt%','Allison et al. (2019) carbon',
                                                                            fail_msg= crmsg_H2O,
    ...
    def calculate_dissolved_volatiles(self,pressure,temperature=1200,sample=None,X_fluid=1.0,**kwargs):
        ...
        sample      NoneType or Sample class
            Magma major element composition. Not required for this model, therefore None may be passed.
        X_fluid     float
        ...
        if self.model_loc not in ['sunset','sfvf','erebus','vesuvius','etna','stromboli']:
            raise core.InputError("model_loc must be one of 'sunset', 'sfvf', 'erebus', 'vesuvius', 'etna', or 'stromboli'.")
    ...
    def root_saturation_pressure(self,pressure,temperature,sample,X_fluid,kwargs):
        ...
        """
        return sample.get_composition('CO2') - self.calculate_dissolved_volatiles(pressure=pressure,temperature=temperature,sample=sample,X_fluid=X_fluid,**kwargs)
...
crmsg_Comp_pass = "The sample appears to be similar in composition to the compositional dataset for the selected Carbon \
model of Allison et al. (2019)."
crmsg_Comp_fail = " These calibration limits were selected based on the minimum and maximum values of these oxides (+-5%) \
in the calibration dataset. As the Allison et al. model incorperates no term for compositional dependence, users must take \
extreme care when extrapolating this model to compositions which differ significantly from the calibration dataset. These \
warnings are simply a guide; we suggest that users carefully compare their major element data to the calibration dataset to \
check for suitability "
crmsg_Comp_description = "The Allison et al. (2019) Carbon model is defined for 6 different alkali compositions."
crmsg_Temp = "All calculations for {model_name} are performed at 1200 C (inputted Temp={param_val:.1f} {units}). \
Allison et al. (2019) suggest the results are likely applicable between 1000-1400°C). "
crmsg_PressureGreater="{param_name} ({param_val:.1f} {units}) is less than the lowest P experiment in the calibration dataset\
 ({calib_val:.1f} {units}). "
crmsg_PressureLess = "{param_name} ({param_val:.1f} {units}) exceeds the upper limit of {calib_val:.1f} {units} suggested by Allison \
et al. (2019) - Their spreadsheet would return 7000 bar for this input. "
crmsg_H2O = "{param_name} ({param_val:.1f} {units}) is > {param_val:.1f} {units}: this model does not account for the effect of H$_2$O \
on volatile solubility. VESIcal allows you to combine Allison Carbon with a variety of H$_2$O models.  "
crmsg_Between_Temp="{param_name} ({param_val:.1f} {units} is outside the recomended temperature range for {model_name} (1000-1400°C). "
...
for ox in sunsetCompRange.keys():
    cr_oxide_list.append(calibration_checks.CalibrationRange(ox, sunsetCompRange[ox], calibration_checks.crf_Between, 'wt%', 'Allison et al. (2019) sunset carbon',
...
sunset.set_calibration_ranges(_crs_to_update+
                 [calibration_checks.CalibrationRange('pressure',7000,calibration_checks.crf_LessThan,'bar','Allison et al. (2019) sunset carbon',
                                                      fail_msg=crmsg_PressureLess,
                                                      pass_msg=calibration_checks.crmsg_LessThan_pass),
                 calibration_checks.CalibrationRange('pressure',4071,calibration_checks.crf_GreaterThan,'bar','Allison et al. (2019) sunset carbon',
                                                      fail_msg=crmsg_PressureGreater,
                                                      pass_msg=calibration_checks.crmsg_GreaterThan_pass),
                calibration_checks.CalibrationRange('sample',None,crf_sunset,None,None,
...
for ox in sfvfCompRange.keys():
    cr_oxide_list.append(calibration_checks.CalibrationRange(ox, sfvfCompRange[ox], calibration_checks.crf_Between, 'wt%', 'Allison et al. (2019) sfvf carbon',
                         fail_msg=calibration_checks.crmsg_BC_fail,
                         pass_msg=calibration_checks.crmsg_BC_pass,
                         description_msg=calibration_checks.crmsg_Between_description))

sfvf.set_calibration_ranges(_crs_to_update+
                 [calibration_checks.CalibrationRange('pressure',7000,calibration_checks.crf_LessThan,'bar','Allison et al. (2019) sfvf carbon',
                                                      fail_msg=crmsg_PressureLess,
                                                      pass_msg=calibration_checks.crmsg_LessThan_pass),
                 calibration_checks.CalibrationRange('pressure',4133,calibration_checks.crf_GreaterThan,'bar','Allison et al. (2019) sfvf carbon',
                                                      fail_msg=crmsg_PressureGreater,
                                                      pass_msg=calibration_checks.crmsg_GreaterThan_pass),
                calibration_checks.CalibrationRange('sample',None,crf_sfvf,None,None,
                                                       fail_msg=crmsg_Comp_fail,
                                                      pass_msg=crmsg_Comp_pass)]
                + cr_oxide_list)
...
for ox in erebusCompRange.keys():
    cr_oxide_list.append(calibration_checks.CalibrationRange(ox, erebusCompRange[ox], calibration_checks.crf_Between, 'wt%', 'Allison et al. (2019) erebus carbon',
                         fail_msg=calibration_checks.crmsg_BC_fail,
                         pass_msg=calibration_checks.crmsg_BC_pass,
                         description_msg=calibration_checks.crmsg_Between_description))

erebus.set_calibration_ranges(_crs_to_update+
                 [calibration_checks.CalibrationRange('pressure',7000,calibration_checks.crf_LessThan,'bar','Allison et al. (2019) erebus carbon',
                                                      fail_msg=crmsg_PressureLess,
                                                      pass_msg=calibration_checks.crmsg_LessThan_pass),
                 calibration_checks.CalibrationRange('pressure',4078,calibration_checks.crf_GreaterThan,'bar','Allison et al. (2019) erebus carbon',
                                                      fail_msg=crmsg_PressureGreater,
                                                      pass_msg=calibration_checks.crmsg_GreaterThan_pass),
                calibration_checks.CalibrationRange('sample',None,crf_erebus,None,None,
                                                       fail_msg=crmsg_Comp_fail,
                                                      pass_msg=crmsg_Comp_pass)]
                + cr_oxide_list)
...
for ox in etnaCompRange.keys():
    cr_oxide_list.append(calibration_checks.CalibrationRange(ox, etnaCompRange[ox], calibration_checks.crf_Between, 'wt%', 'Allison et al. (2019) etna carbon',
                         fail_msg=calibration_checks.crmsg_BC_fail,
                         pass_msg=calibration_checks.crmsg_BC_pass,
                         description_msg=calibration_checks.crmsg_Between_description))
...
etna.set_calibration_ranges(_crs_to_update+
                 [calibration_checks.CalibrationRange('pressure',7000,calibration_checks.crf_LessThan,'bar','Allison et al. (2019) etna carbon',
                                                      fail_msg=crmsg_PressureLess,
                                                      pass_msg=calibration_checks.crmsg_LessThan_pass),
                 calibration_checks.CalibrationRange('pressure',485,calibration_checks.crf_GreaterThan,'bar','Allison et al. (2019) etna carbon',
                                                      fail_msg=crmsg_PressureGreater,
                                                      pass_msg=calibration_checks.crmsg_GreaterThan_pass),
                calibration_checks.CalibrationRange('sample',None,crf_etna,None,None,
                                                       fail_msg=crmsg_Comp_fail,
                                                      pass_msg=crmsg_Comp_pass)]
                + cr_oxide_list)
...
for ox in vesuviusCompRange.keys():
    cr_oxide_list.append(calibration_checks.CalibrationRange(ox, vesuviusCompRange[ox], calibration_checks.crf_Between, 'wt%', 'Allison et al. (2019) vesuvius carbon',
                         fail_msg=calibration_checks.crmsg_BC_fail,
                         pass_msg=calibration_checks.crmsg_BC_pass,
                         description_msg=calibration_checks.crmsg_Between_description))

vesuvius.set_calibration_ranges(_crs_to_update+
                 [calibration_checks.CalibrationRange('pressure',7000,calibration_checks.crf_LessThan,'bar','Allison et al. (2019) vesuvius carbon',
                                                      fail_msg=crmsg_PressureLess,
                                                      pass_msg=calibration_checks.crmsg_LessThan_pass),
                 calibration_checks.CalibrationRange('pressure',269,calibration_checks.crf_GreaterThan,'bar','Allison et al. (2019) vesuvius carbon',
                                                      fail_msg=crmsg_PressureGreater,
                                                      pass_msg=calibration_checks.crmsg_GreaterThan_pass),
                calibration_checks.CalibrationRange('sample',None,crf_vesuvius,None,None,
                                                       fail_msg=crmsg_Comp_fail,
                                                      pass_msg=crmsg_Comp_pass)]
                + cr_oxide_list)
...
for ox in stromboliCompRange.keys():
    cr_oxide_list.append(calibration_checks.CalibrationRange(ox, stromboliCompRange[ox], calibration_checks.crf_Between, 'wt%', 'Allison et al. (2019) stromboli carbon',
                         fail_msg=calibration_checks.crmsg_BC_fail,
                         pass_msg=calibration_checks.crmsg_BC_pass,
                         description_msg=calibration_checks.crmsg_Between_description))

stromboli.set_calibration_ranges(_crs_to_update+
                 [calibration_checks.CalibrationRange('pressure',7000,calibration_checks.crf_LessThan,'bar','Allison et al. (2019) stromboli carbon',
                                                      fail_msg=crmsg_PressureLess,
                                                      pass_msg=calibration_checks.crmsg_LessThan_pass),
                 calibration_checks.CalibrationRange('pressure',524,calibration_checks.crf_GreaterThan,'bar','Allison et al. (2019) stromboli carbon',
                                                      fail_msg=crmsg_PressureGreater,
                                                      pass_msg=calibration_checks.crmsg_GreaterThan_pass),
                calibration_checks.CalibrationRange('sample',None,crf_stromboli,None,None,
                                                       fail_msg=crmsg_Comp_fail,
                                                      pass_msg=crmsg_Comp_pass)]
                + cr_oxide_list)
