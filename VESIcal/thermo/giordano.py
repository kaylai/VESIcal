from VESIcal import core, sample_class
...
def calculate_liquid_viscosity(sample, temperature, **kwargs):
    ...
    B_value = sum(
            [_calculate_partial_B_value(comp_molpercent,b) for b in b_numbers])

    c_numbers = [1, 2, 3, 4, 5, 6, 11]
    C_value = sum(
            [_calculate_partial_C_value(comp_molpercent,c) for c in c_numbers])
