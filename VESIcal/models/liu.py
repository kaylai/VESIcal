...
class water(model_classes.Model):
    ...
    def __init__(self):
        ...
        for ox in watercomprange.keys():
            cr_oxide_list.append(calibration_checks.CalibrationRange(ox, watercomprange[ox], calibration_checks.crf_Between, 'wt%', 'Liu et al. (2005) water',
        ...
        self.set_calibration_ranges([calibration_checks.CalibrationRange('pressure',[0,5000.0],calibration_checks.crf_Between,'bar','Liu et al. (2005) water',
                                                      fail_msg=calibration_checks.crmsg_Between_fail,
                                                      pass_msg=calibration_checks.crmsg_Between_pass,
                                                      description_msg=calibration_checks.crmsg_Between_description),
                                     calibration_checks.CalibrationRange('temperature',[700.0,1200],calibration_checks.crf_Between,'oC','Liu et al. (2005) water',
                                                      fail_msg=calibration_checks.crmsg_Between_fail,
                                                      pass_msg=calibration_checks.crmsg_Between_pass,
                                                      description_msg=calibration_checks.crmsg_Between_description),
                                     calibration_checks.CalibrationRange('sample',None,crf_WaterComp,None,None,
...
class carbon(model_classes.Model):
    ...
    def __init__(self):
        ...
        for ox in carboncomprange.keys():
            cr_oxide_list.append(calibration_checks.CalibrationRange(ox, carboncomprange[ox], calibration_checks.crf_Between, 'wt%', 'Liu et al. (2005) carbon',
                                 fail_msg=calibration_checks.crmsg_BC_fail,
                                 pass_msg=calibration_checks.crmsg_BC_pass,
                                 description_msg=calibration_checks.crmsg_Between_description))

        self.set_calibration_ranges([calibration_checks.CalibrationRange('pressure',[0,5000.0],calibration_checks.crf_Between,'bar','Liu et al. (2005) Carbon',
                                                      fail_msg=calibration_checks.crmsg_Between_fail,
                                                      pass_msg=calibration_checks.crmsg_Between_pass,
                                                      description_msg=calibration_checks.crmsg_Between_description),
                                     calibration_checks.CalibrationRange('temperature',[700.0,1200],calibration_checks.crf_Between,'oC','Liu et al. (2005) Carbon',
                                                       fail_msg=calibration_checks.crmsg_Between_fail,
                                                      pass_msg=calibration_checks.crmsg_Between_pass,
                                                      description_msg=calibration_checks.crmsg_Between_description),
                                     calibration_checks.CalibrationRange('sample',None,crf_CarbonComp,None,None,
    ...
    def root_saturation_pressure(self,pressure,temperature,sample,X_fluid,kwargs):
        ...
        float
            ...
            passed in the sample variable.
        """
        return self.calculate_dissolved_volatiles(pressure=pressure,temperature=temperature,sample=sample,X_fluid=X_fluid,**kwargs) - sample.get_composition('CO2')
...
def crf_WaterComp(calibval=None,sample=sample_class.Sample({})):
    ...
    test_results = []
    for ox in watercomprange.keys():
        test_results.append(comp[ox] >= watercomprange[ox][0] and comp[ox] <= watercomprange[ox][1])
...
def crf_CarbonComp(calibval=None,sample=sample_class.Sample({})):
    ...
    test_results = []
    for ox in carboncomprange.keys():
        test_results.append(comp[ox] >= carboncomprange[ox][0] and comp[ox] <= carboncomprange[ox][1])
...
crmsg_Comp_pass = "The sample appears to be similar in composition to the rhyolites and haplogranites used to calibrate the Liu et al. model."
crmsg_WaterComp_fail = " These calibration limits were selected based on the minimum and maximum values of these oxides (+-5%) in the Water calibration dataset. \
As the Liu et al. model incorperates no term for compositional dependence, users must take extreme care when extrapolating this model to compositions which differ \
significantly from the haplogranites and rhyolites in the calibration dataset. These warnings are simply a guide; we suggest that users carefully compare their major \
element data to the calibration dataset to check for suitability "
crmsg_CarbonComp_fail = " These calibration limits were selected based on the minimum and maximum values of these oxides (+-5%) in the Carbon calibration dataset. \
As the Liu et al. model incorperates no term for compositional dependence, users must take extreme care when extrapolating this model to compositions which differ \
significantly from the haplogranites and rhyolites in the calibration dataset. These warnings are simply a guide; we suggest that users carefully compare their major \
element data to the calibration dataset to check for suitability "
crmsg_Comp_fail = " These calibration limits were selected based on the minimum and maximum values of these oxides (+-5%) in the combined Water and Carbon \
calibration dataset. As the Liu et al. model incorperates no term for compositional dependence, users must take extreme care when extrapolating this model to \
compositions which differ significantly from the haplogranites and rhyolites in the calibration dataset. These warnings are simply a guide; we suggest that users \
carefully compare their major element data to the calibration dataset to check for suitability "
...
for ox in carboncomprange.keys():
    ...
    cr_oxide_list.append(calibration_checks.CalibrationRange(ox, [lo,hi], calibration_checks.crf_Between, 'wt%', 'Liu et al. (2005)',
                         fail_msg=calibration_checks.crmsg_BC_fail,
                         pass_msg=calibration_checks.crmsg_BC_pass,
                         description_msg=calibration_checks.crmsg_Between_description))

mixed.models[0].set_calibration_ranges([calibration_checks.CalibrationRange('pressure',[0,5000.0],calibration_checks.crf_Between,'bar','Liu et al. (2005)',
                                                      fail_msg=calibration_checks.crmsg_Between_fail,
                                                      pass_msg=calibration_checks.crmsg_Between_pass,
                                                      description_msg=calibration_checks.crmsg_Between_description),
                                     calibration_checks.CalibrationRange('temperature',[700.0,1200],calibration_checks.crf_Between,'oC','Liu et al. (2005)',
                                                       fail_msg=calibration_checks.crmsg_Between_fail,
                                                      pass_msg=calibration_checks.crmsg_Between_pass,
                                                      description_msg=calibration_checks.crmsg_Between_description),
                                     calibration_checks.CalibrationRange('sample',None,crf_MixedComp,None,None,
